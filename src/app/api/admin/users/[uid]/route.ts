...
export async function GET(
  ...
  try {
    ...
    const user = {
      ...
      fit_profile: userData.fit_profile ?? null,
      emailTags: Array.isArray(userData.emailTags) ? userData.emailTags : [],
      subscription_status: subs.status ?? "none",
      mullybox_active: subs.mullybox_active ?? false,
      manage_url: subs.manage_url ?? null,
      active_subscription_ids: Array.isArray(subs.active_subscription_ids) ? subs.active_subscription_ids : [],
      store_credit_cents: typeof credit.balance_cents === "number" ? credit.balance_cents : 0,
